...
describe(testing.suiteName(), function() {
  describe('offset/limit', function() {
    ...
    it('paginate with continuationToken and limit ending at returned rows', async function() {
      const continuationToken = hashids.encode(5);
      assert.deepEqual(
    ...
    it('paginate with continuationToken and limit returning another page', async function() {
    ...
    it('paginate with invalid continuationToken throws error', async function() {
  ...
  describe('index-based', function() {
    ...
    it('paginate with a limit higher than returned rows', async function() {
      assert.deepEqual(
        await paginateResults({ query: { limit: 110 }, indexColumns, fetch: fetcher(20, 5) }),
        { rows: data(20, 5).slice(0, 110) });
    });

    it('paginate with a limit equal to returned rows', async function() {
      assert.deepEqual(
        await paginateResults({ query: { limit: 100 }, indexColumns, fetch: fetcher(20, 5) }),
        { rows: data(20, 5) });
    });

    it('paginate with a limit one less than returned rows', async function() {
      assert.deepEqual(
    ...
    it('iterate over entire data set', async function() {
      let got = [];
      const query = { limit: 1 };
      while (true) {
    ...
    it('invalid base64 in token starts at first row', async function() {
    ...
    it('invalid JSON in token starts at first row', async function() {
    ...
    it('non-array JSON in token starts at first row', async function() {
      const continuationToken = Buffer.from('{"abc": 123}').toString('base64');
      assert.deepEqual(
        await paginateResults({ query: { limit: 1, continuationToken }, indexColumns, fetch: fetcher(5, 5) }),
        { rows: data(5, 5).slice(0, 1), continuationToken: token(0, 0) });
    ...
    it('bad-length array JSON in token starts at first row', async function() {
